...
export const DownloadMediaFilesTask: TaskProc.DownloadMediafileReqHandlerSingle = async ({ payload, signal, emit }) => {
	...
	try {
		const file = payload;
		if(!file){
			throw new Error('Download file failed. Expected media file');
		}
		...
		emit({
			type: 'progress',
			message: `Downloading file started, '${file.source.title}'`,
			payload: {
				fileId: file.id,
				stage: 1
			},
		...
		for (let trackIndex = 0; trackIndex < file.trackIds.length; trackIndex++) {
			...
			emit({
				...
				message: `Downloading file ${trackIndex + 1} track: ${track.formatId}, '${file.source.title}'`,
				payload: {
					fileId: file.id,
					stage: 2
				},
			...
			emit({
				type: 'progress',
				message: `Downloaded file ${trackIndex + 1} track: ${track.formatId}, '${file.source.title}'`,
				payload: {
					fileId: file.id,
					stage: 3
				},
			...
			emit({
				type: 'progress',
				message: `ffmpeg is merging file, '${file.source.title}'`,
				payload: {
					fileId: file.id,
					stage: 4
				},
			...
			emit({
				type: 'progress',
				message: `File done, ${file.source.title}`,
				payload: {
					stage: 5,
					fileId: file.id,
